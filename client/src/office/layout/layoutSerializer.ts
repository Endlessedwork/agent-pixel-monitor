...
export function layoutToFurnitureInstances(furniture: PlacedFurniture[]): FurnitureInstance[] {
  ...
  for (const item of furniture) {
    ...
    if (!entry || (!entry.isDesk && entry.category !== 'desks')) continue;
...
export function layoutToSeats(furniture: PlacedFurniture[]): Map<string, Seat> {
  ...
  // Work desk tiles = desks with backgroundTiles (real work surfaces, not coffee tables)
  const workDeskTiles = new Set<string>();
  for (const item of furniture) {
    const entry = getCatalogEntry(item.type);
    if (!entry || (!entry.isDesk && entry.category !== 'desks')) continue;
    const isWorkDesk = (entry.backgroundTiles ?? 0) > 0;
    for (let dr = 0; dr < entry.footprintH; dr++) {
      for (let dc = 0; dc < entry.footprintW; dc++) {
        const key = `${item.col + dc},${item.row + dr}`;
        deskTiles.add(key);
        if (isWorkDesk) workDeskTiles.add(key);
      }
    }
  }
  ...
  for (const item of furniture) {
    ...
    let seatCount = 0;
    for (let dr = 0; dr < entry.footprintH; dr++) {
      if (dr < bgRows) continue; // skip background rows (e.g. chair backrest)
      for (let dc = 0; dc < entry.footprintW; dc++) {
        ...
        // Check adjacentDesk using work desks only (not coffee tables)
        let adjacentDesk = false;
        for (const d of dirs) {
          if (workDeskTiles.has(`${tileCol + d.dc},${tileRow + d.dr}`)) {
            adjacentDesk = true;
            break;
          }
        }
        if (entry.orientation) {
          ...
          // Face toward any adjacent desk (including non-work desks)
          for (const d of dirs) {
...
function migrateLayout(layout: OfficeLayout): OfficeLayout {
  ...
  const migratedLayout = { ...layout, furniture: migrateFurnitureTypes(layout.furniture) };

  // Migrate old VOID value (was 8, now 255) -- only for legacy layouts since FLOOR_8 reuses value 8
  const OLD_VOID = 8;
  let result = migratedLayout;
  if (!result.layoutRevision && result.tiles.includes(OLD_VOID as TileTypeVal)) {
    result = {
      ...result,
      tiles: result.tiles.map((t) => (t === OLD_VOID ? (TileType.VOID as TileTypeVal) : t)),
    };
  }

  if (result.tileColors && result.tileColors.length === result.tiles.length) {
    return result; // Already migrated tile colors
  }
